...
public abstract class PowerButton {
    ...
    // a static onclicklistener that can be set to register a callback when ANY
    // button is clicked
    private static View.OnClickListener GLOBAL_ON_CLICK_LISTENER = null;

    // a static onlongclicklistener that can be set to register a callback when
    // ANY button is long clicked
    ...
    private Handler mViewUpdateHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (mView != null) {
                ...
                Slog.i(TAG, "updating icon");
                /* Button State */
                switch (mState) {
                    case STATE_ENABLED:
                        updateImageView(buttonState,
                                getDrawable(R.drawable.stat_bgon_custom, sColorMaskOn, MASK_MODE));
                        break;
                    case STATE_DISABLED:
                        updateImageView(buttonState,
                                getDrawable(R.drawable.stat_bgon_custom, sColorMaskOff, MASK_MODE));
                        break;
                    default:
                        updateImageView(
                                buttonState,
                                getDrawable(R.drawable.stat_bgon_custom, sColorMaskInter, MASK_MODE));
                        break;
                }
            }
        }

        private Drawable getDrawable(int id, int mask, Mode maskType) {
            Drawable tmp = res.getDrawable(id);

            tmp.setColorFilter(mask, maskType);
            return tmp;
        }
    };
